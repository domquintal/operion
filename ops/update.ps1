...
$parts=$ver.Split('.').ForEach({[int]$_}); switch($Bump){"major"{$parts[0]++;$parts[1]=0;$parts[2]=0}"minor"{$parts[1]++;$parts[2]=0}default{$parts[2]++}}
...
if($ok){
  git add -A; git commit -m "chore: update → sanity PASS ($new)" | Out-Null
  git push origin $b; Write-Log "DONE ✅ (sanity PASS)" "OK" $log; exit 0
}else{ Write-Log "ABORT ❌ (sanity FAIL). Not pushing." "ERROR" $log; exit 2 }
