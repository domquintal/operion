...
if ($repo) {
  ...
  Add-Content -LiteralPath $log -Value ("branch: {0} @ {1}" -f (Git-Branch), (Git-Short))
}

# VERSION.txt semantic
...
# logs writable
try {
  $probeDir = Join-Path $ScriptDir "..\_logs"; New-Item -ItemType Directory -Force -Path $probeDir | Out-Null
...
# settings valid json
$setPath = Join-Path $ScriptDir "..\app\settings.json"
try { $null = Read-Json $setPath; $jok = $true } catch { $jok = $false }
AddCheck $jok "settings.json valid"

# app runner exists
AddCheck (Test-Path (Join-Path $ScriptDir "..\app\run.ps1")) "app\run.ps1 present"
