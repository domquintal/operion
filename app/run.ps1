...
$logDir = Join-Path $Repo "_logs"; New-Item -ItemType Directory -Force -Path $logDir | Out-Null
...
if ($keep -gt 0) {
  Get-ChildItem $logDir -Filter "app_*.log" -File | Where-Object { $_.LastWriteTime -lt (Get-Date).AddDays(-$keep) } | Remove-Item -Force -ErrorAction SilentlyContinue
}
