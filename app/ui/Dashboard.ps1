...
# repo root (two levels up from script)
...
function Get-LatestLog {
  ...
  Get-ChildItem $Logs -File | Sort-Object LastWriteTime -Desc | Select-Object -First 1
}
function Analyze {
  $lf = Get-LatestLog
  ...
  $BeatTxt.Text = $beats.ToString()
  $ErrTxt.Text  = $errs.ToString()
  $LatestTxt.Text = ($lines | Select-Object -Last 1)
}
$timer = New-Object Windows.Threading.DispatcherTimer; $timer.Interval=[TimeSpan]::FromSeconds(2)
$timer.Add_Tick({ Analyze })
$timer.Start(); Analyze
